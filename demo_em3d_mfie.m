...
M = sqrt(triaarea)';
M = repmat(M,1,2);
M = reshape(M,1,2*nsource)';
%
%  Call the solver
%
'Interior/Exterior MFIE solver for the Maxwell equations in R^3'
tic
rhs0 = reshape(rhs,1,2*nsource);
sol0 = gmres_simple(@(x) em3dmultfmmflat_mfie(A,x), rhs0.', 1e-2, 20);
%%%sol0 = gmres_simple(@(x) M.*em3dmultfmmflat_mfie(A,x./M), M.*rhs0.', 1e-2, 20)./M;
sol0 = sol0.';
sol = reshape(sol0,2,nsource);
time_gmres=toc

sol = repmat(sol(1,:),3,1).*A.triatang1+repmat(sol(2,:),3,1).*A.triatang2;
